...
inline fun <T> Result<T>.getResult(
    success: (Result.Success<T>) -> Result<T>,
    error: (Result.Error<T>) -> Result<T>
): Result<T> = if (this is Result.Success) success(this) else error(this as Result.Error)
